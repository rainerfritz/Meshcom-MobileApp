...
const NodeMap = () => {
  ...
  const [ markerInfo, setMarkerInfo] = useState({ lat_: 48.20579, lon_: 16.37160, alt_:243, call_:"OE1KFR-4", bat_:"100", hw_:"RAK4631", press_:0, hum_:0, temp_:0, qnh_:0, timestamp_:0, comment_:"", name_:"", temp_2_:0, co2_:0, gas_res_:0, alt_press_:0, neighbour_count_:0, groups_:"", symbol_table_:"", symbol_:"",
    din_:undefined as string|null|undefined, vbus_:undefined as number|null|undefined, vcurrent_:undefined as number|null|undefined });
  ...
  const handleShowMarkerInfo = (e:any, i:any, call:string, lat:number, lon:number, alt:number, bat:string, hw:string,
    pressure:number, humidity:number, temperature:number, qnh:number, timestamp:number, comment:string, name:string|undefined, temp_2:number, co2:number,
    gas_res:number, alt_press:number, neighbour_count:number, groups:string, symbol_table:string, symbol:string,
    din?:string|null, vbus?:number|null, vcurrent?:number|null) => {
    ...
    const info = {call_:call, lat_:lat, lon_:lon, alt_:alt, bat_:bat, hw_:hw, press_:pressure, hum_:humidity, temp_:temperature, qnh_:qnh,
      timestamp_:timestamp, comment_:comment, name_:name ?? "", temp_2_:temp_2, co2_:co2, gas_res_:gas_res, alt_press_:alt_press, neighbour_count_:neighbour_count, groups_:groups,
      symbol_table_:symbol_table, symbol_:symbol, din_:din, vbus_:vbus, vcurrent_:vcurrent};
    // print the info
  ...
  const goToCallsign = (sPos:PosType) =>{
    ...
    handleShowMarkerInfo(1,2,sPos.callSign, sPos.lat,sPos.lon,sPos.alt,sPos.bat,sPos.hw,sPos.pressure,sPos.humidity,
      sPos.temperature,sPos.qnh,sPos.timestamp, sPos.comment, sPos.name, sPos.temp_2, sPos.co2, sPos.gas_res, sPos.alt_press, sPos.neighbour_count, sPos.groups,
      sPos.symbol_table, sPos.symbol, sPos.din, sPos.vbus, sPos.vcurrent);
            ...
            {showMarkers ? positions.map((pos, i) => (
              <Marker
                ...
                onClick={e => handleShowMarkerInfo(e, i, pos.callSign, pos.lat, pos.lon, pos.alt, pos.bat, pos.hw, pos.pressure, pos.humidity,
                  pos.temperature, pos.qnh, pos.timestamp, pos.comment, pos.name, pos.temp_2, pos.co2, pos.gas_res, pos.alt_press, pos.neighbour_count, pos.groups,
                  pos.symbol_table, pos.symbol, pos.din, pos.vbus, pos.vcurrent )}
                width={50}
            ...
            {showCurrentPointInfo &&
              <Overlay anchor={[currentPoint.latitude, currentPoint.longitude]} offset={[110, 340]}>
                <MapOverlay callSign={markerInfo.call_} lat={markerInfo.lat_} lon={markerInfo.lon_} alt={markerInfo.alt_} bat={markerInfo.bat_}
                  ...
                  comment={markerInfo.comment_} name={markerInfo.name_} temp_2={markerInfo.temp_2_} co2={markerInfo.co2_} gas_res={markerInfo.gas_res_} alt_press={markerInfo.alt_press_}
                  neighbour_count={markerInfo.neighbour_count_} groups={markerInfo.groups_}
                  symbol_table={markerInfo.symbol_table_} symbol={markerInfo.symbol_}
                  din={markerInfo.din_ ?? undefined} vbus={markerInfo.vbus_ ?? undefined} vcurrent={markerInfo.vcurrent_ ?? undefined}
